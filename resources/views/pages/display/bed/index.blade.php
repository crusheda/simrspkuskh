        ...
        <div class="row align-items-center">
            <div class="col-md-12">
                <ul class="breadcrumb">
                    ...
                    <li class="breadcrumb-item" aria-current="page">Tempat Tidur</li>
                </ul>
            </div>
            <div class="col-md-12">
                <div class="page-header-title">
                    <h2 class="mb-0">Display <b class="text-primary">Tempat Tidur Rawat Inap</b></h2>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card-header align-items-center d-flex justify-content-between py-2 px-0">
            <div>
                <div class="input-group">
                    <select class="form-select" id="jenis_display">
                        <option value="">Pilih Poliklinik</option>
                        <option value="bkelas">Berdasarkan Kelas</option>
                        <option value="bruangan">Berdasarkan Ruangan</option>
                    </select>
                    <button class="btn btn-primary btn-wave me-1 waves-effect waves-light" id="tampil_display" onclick="refresh($('#jenis_display').val())" disabled>
...
<script>
    ...
    $('#tampil_display').on('click', function() {
        let jenis = $('#jenis_display').val();
        refresh(jenis);
    });

    $('#jenis_display').on('change', function () {
        const selDr = $(this).val();
        if (!selDr) {
            $('#tampil_display').prop('disabled',true);
        } else {
            $('#tampil_display').prop('disabled',false);
        };
    });

    function refresh(jenis = null) {
        if (jenis !== null) {
            lastJenis = jenis;     // simpan jenis terakhir
        }
        ...
        $.ajax({
            url: "/api/display/tt",
            type: "GET",
            data: { jenis: lastJenis },   // kirim jenis
            dataType: "json",
            success: function(res) {
                ...
                // Set header dinamis
                $("#tampil-tbody").closest('table').find('thead').html(theadHtml);
                // ============================
                ...
                // ============================
                // MODE 2 = BERDASARKAN RUANGAN + KAMAR
                // ============================
                else if (res.mode === "bruangan") {

                    res.bangsal.forEach(item => {
                        let firstBangsal = true;

                        Object.values(item.kelas).forEach(kls => {
                            let firstKelas = true;

                            kls.kamar.forEach(kamar => {
                                html += "<tr>";

                                if (firstBangsal && firstKelas) {
                                    html += `<td rowspan="${item.total_kamar}" class="fw-bold bg-light">${item.bangsal}</td>`;
                                }

                                if (firstKelas) {
                                    html += `<td rowspan="${kls.kamar.length}" class="fw-bold">${kls.nama_kelas}</td>`;
                                    firstKelas = false;
                                }

                                html += `
                                    <td>${kamar.nama_kamar}</td>
                                    <td class="text-primary fw-bold">${kamar.kosong}</td>
                                    <td class="text-success fw-bold">${kamar.terpesan}</td>
                                    <td class="text-danger fw-bold">${kamar.terisi}</td>
                                    <td class="fw-bold bg-light">${kamar.total_bed}</td>
                                </tr>`;
                            });
