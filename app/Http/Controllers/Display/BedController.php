...
class BedController extends Controller
{
    ...
    function getDisplayTt(Request $r)
    {
        $jenis = $r->jenis ?? 'bkelas'; // default bkelas
        ...
        foreach ($bangsalList as $b) {
            ...
            // ======================================================
            // MODE 1 — BERDASARKAN KELAS (SEPERTI SKRIP ASLI)
            // ======================================================
            if ($jenis === "bkelas") {
            ...
            // ======================================================
            // MODE 2 — BERDASARKAN RUANGAN → KELAS → KAMAR
            // ======================================================
            if ($jenis === "bruangan") {

                $kamarList = DB::table('master.ruang_kamar')
                    ->where('RUANGAN', $b->ID)
                    ->select('ID', 'KAMAR as nama_kamar', 'KELAS')
                    ->whereIn('STATUS', [1, 2, 3])
                    ->get();
                $kelasGrouped = [];

                foreach ($kamarList as $kam) {

                    $tt = DB::table('master.ruang_kamar_tidur')
                        ->where('RUANG_KAMAR', $kam->ID)
                        ->selectRaw('
                    ...
                    $kelasGrouped[$kam->KELAS]['kamar'][] = [
                        'nama_kamar' => $kam->nama_kamar,
                        'kosong'     => $tt->kosong,
                        'terpesan'   => $tt->terpesan,
                        'terisi'     => $tt->terisi,
                        'total_bed'  => $tt->total_bed
                    ];
                }

                // Tambahkan nama kelas
                foreach ($kelasGrouped as $kelasId => $x) {
                    $ref = DB::table('master.referensi')
                            ->where('JENIS', 19)
                            ->where('ID', $kelasId)
                            ->first();

                    $kelasGrouped[$kelasId]['nama_kelas'] = $ref ? $ref->DESKRIPSI : '-';
                }

                $totalKamar = 0;
                foreach ($kelasGrouped as $x) {
                    $totalKamar += count($x['kamar']);
                }
