...
class AntrianAdmisiController extends Controller
{
    function index()
    {
        ...
        $ruang = DB::table('regonline.pos_antrian AS pa')
            ->select('pa.NOMOR AS IDRUANGAN','pa.DESKRIPSI AS NAMARUANGAN')
            ->where('pa.STATUS',1)
            ->where('pa.PANGGIL',1)
            ->get();
        ...
        // print_r($data);
        // die();
        return view('pages.display.antrian.admisi.index')->with('list', $data);
    }

    function getDisplayAntrianAdmisi(Request $request)
    {
        $poli = $request->input('poli1');
